...
import { Actor, Producer, Users } from "@/app/Types/entitytypes";
...
export default function Info ({ params } : { params : Promise<{ n : number }> }) {
    ...
    async function getAssociated() : Promise<Array<{ id: number, name: string, date_released: string }>> {
        ...
        let films = await getFilmActors(undefined, n);

        for (let i = 0; i <= films.length - 1; i++) {
            let film = await getFilm(films[i].film_fk!);
    ...
    if (!actor || !films) {
        return(
            <>
                <Header currentUser={ undefined } loading={!actor || !films} />
            ...
            <Box className='break-words flex flex-col gap-5 md:min-h-[100vh] md:max-w-[50vw] w-[50vw] mx-auto bg-black/30 p-6 rounded-lg text-white backdrop-blur-sm'>
                <Box className='flex flex-row justify-between'>
                    ...
                    <Tooltip title='Back to Actor Page'>
                        <Button variant='outlined' onClick={() => navigate.push('/actors')}>
                            <ArrowBackIcon />
                        </Button>
                    </Tooltip>
                    </Box>
                ...
                <Box><Typography variant='h6'>Sex: { actor.sex.toUpperCase() }</Typography></Box>
                <Box className='flex flex-row justify-between'>
                    <Typography>Birthdate: { new Date(actor.birthday).toDateString().substring(4) }</Typography>
                </Box>
                ...
                <Box className='mt-[4vh] min-w-0'>
                    <Typography variant='h5'>Social Media</Typography>
                    <Box className='mt-[2vh] flex flex-col min-w-0'>
                        { formatText(actor.socmed, true) }    
                    </Box>
                </Box>

                <Box className='flex flex-row gap-20 mt-[4vh]'>
                    <Box className='max-w-[40vw]'>
                        <Typography variant='h5'>Films Appeared In</Typography>
