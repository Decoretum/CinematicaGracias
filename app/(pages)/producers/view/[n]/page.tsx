...
import { Producer, Users } from "@/app/Types/entitytypes";
...
export default function Info ({ params } : { params : Promise<{ n : number }> }) {
    ...
    async function getAssociated() : Promise<Array<{ id: number, name: string, date_released: string }>> {
        ...
        let films = await getFilmProducers(undefined, n);
    ...
    if (!producer || !films) {
        return(
            <>
                <Header currentUser={ undefined } loading={!producer || !films} />
            ...
            <Box className='break-words flex flex-col gap-5 md:min-h-[100vh] md:max-w-[50vw] w-[50vw] mx-auto bg-black/30 p-6 rounded-lg text-white backdrop-blur-sm'>
                <Box className='flex flex-row justify-between'>
                    ...
                    <Tooltip title='Back to Producer Page'>
                        <Button variant='outlined' onClick={() => navigate.push('/producers')}>
                            <ArrowBackIcon />
                        </Button>
                    </Tooltip>
                    </Box>
                ...
                <Box><Typography variant='h6'>Sex: { producer.sex.toUpperCase() }</Typography></Box>
                <Box className='flex flex-row justify-between'>
                    <Typography>Birthdate: { new Date(producer.birthday).toDateString().substring(4) }</Typography>
                </Box>

                <Box className='flex flex-col gap-4 mt-[4vh]'>
                    <Typography variant='h4'> Overview </Typography>
                    <Typography variant='body2'> { producer.description } </Typography>
                </Box>

                <Box className='flex flex-row gap-20 mt-[4vh]'>
                    <Box className='max-w-[40vw]'>
                        <Typography variant='h5'>Films Produced</Typography>
